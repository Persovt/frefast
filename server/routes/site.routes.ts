...
SiteRouter.post("/cheackSite", async function (req, res) {
  ...
  const candidat = await SiteShema.findOne({ name: siteName });

  if (!candidat) res.status(400).json({ message: "Site not found " });
...
SiteRouter.post("/addSite", async function (req, res) {
  ...
  const candidat = await SiteShema.findOne({ name: siteName });
  const user = await RefreshModel.findOne({ refreshToken });
  console.log(user.userId);

  if (candidat) res.status(400).json({ message: "Name is zanato" });
