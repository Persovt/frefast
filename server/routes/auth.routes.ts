    ...
    try{
        ...
        if(user.password !== sha512.hmac('password', password))
            ...
            //console.log(cons.length,cons)
             if(cons.length > 4){
                  RefreshModel.deleteOne({refreshToken: cons[0].refreshToken}, function(err){
                    if(err) throw err;
                }); 
            }
        ...
        const accesToken = jwt.sign({userId: user.id, email: user.email, role: user.role}, 'shhhh', { expiresIn: '30m' }) //30m
        console.log("login")
        res.cookie('accesToken', accesToken, { maxAge: 30 * 60 * 1000, security: true })
        res.cookie('refreshToken', refreshToken, {maxAge: 60 * 24 * 60 * 60 * 1000, httpOnly: true, security: true})
...
AuthRouter.post('/refresh-tokens', async function(req, res) {
    try{
        ...
        if(candidat[0].fingerprint !== visitorId){
            ...
            res.cookie('refreshToken', '', {maxAge: 0, httpOnly: true, security: true})
...
//           const accesToken = jwt.sign({userId: candidat.userId}, 'shhhh', { expiresIn: '30m' }) //30m
           ...
           RefreshModel.deleteOne({refreshToken: candidat[0].refreshToken}, function(err){ if(err) throw err;   }); 

            
            console.log(accesToken,refreshTokenNew, "create")
             res.cookie('accesToken', accesToken, { maxAge: 30 * 60 * 1000, security: true })
            res.cookie('refreshToken', refreshTokenNew, {maxAge: 60 * 24 * 60 * 60 * 1000, httpOnly: true, security: true})
